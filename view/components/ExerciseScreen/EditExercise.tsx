...
const EditExercise = ({ visible, onClose, onSubmit, exercise_id }: EditExerciseProps) => {
  ...
  const fetchCurrentExercise = async () => {
    try {
      const response = await axiosPrivate.get(`/api/exercises/exercise/${exercise_id}`);
      setExerciseName(response.data.result[0].exercise_name)
      setSelectedBodyPart(response.data.result[0].exercise_bodypart)
      setSelectedCategory(response.data.result[0].exercise_category)
      setInstructions(response.data.result[0].exercise_instructions)
    } catch (error) {
      console.error(error)
    }
  }
        ...
        <View className="bg-surface rounded-3xl border-2 border-accent/30 w-full max-w-lg">
          ...
          <ScrollView className="px-6 pb-6" showsVerticalScrollIndicator={false}>
            ...
            <View className="flex-row gap-3">
              ...
              >
                <Text className="text-white font-psemibold text-base">Edit Exercise</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
