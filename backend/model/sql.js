...
const updateUserProfile = async (prevName, name, phone, email, imageUrl, profileImageKey) => {
    const { data, error } = await supabase
        .from('users')
        .update({ user_name: name, user_email: email, user_phone: phone, profile_image: imageUrl, profile_image_key: profileImageKey })
...
const findUserSecure = async (user) => {
    const { data, error } = await supabase
        .from('users')
        .select('user_name, user_email, id, user_phone, profile_image, profile_image_key')
        .eq('user_name', user);
    if (error) throw error;
    return data;
};
